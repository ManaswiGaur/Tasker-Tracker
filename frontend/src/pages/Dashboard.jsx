  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <MetricCard label="Workspaces" count={stats?.totalProjects || 0} icon={<FolderKanban size={18} />} color="text-[var(--accent)]" bg="bg-purple-500/10 dark:bg-purple-500/15" hoverBorder="hover:border-purple-500/30" />
        <MetricCard label="Completed" count={stats?.completedTasks || 0} icon={<CheckCircle2 size={18} />} color="text-emerald-500" bg="bg-emerald-500/10 dark:bg-emerald-500/15" hoverBorder="hover:border-emerald-500/30" />
        <MetricCard label="Active" count={stats?.pendingTasks || 0} icon={<Clock3 size={18} />} color="text-amber-500" bg="bg-amber-500/10 dark:bg-amber-500/15" hoverBorder="hover:border-amber-500/30" />
        <MetricCard label="Overdue" count={stats?.overdueTasks || 0} icon={<AlertTriangle size={18} />} color="text-rose-500" bg="bg-rose-500/10 dark:bg-rose-500/15" hoverBorder="hover:border-rose-500/30" />
      </div>
...
function MetricCard({ label, count, icon, color, bg, hoverBorder }) {
  return (
    <div className={`rounded-2xl card p-4 hover:-translate-y-0.5 hover:shadow-lg hover:bg-[var(--surface-raised)] transition-all duration-300 group ${hoverBorder}`}>
      <div className="flex items-center gap-3">
        <div className={`flex h-10 w-10 items-center justify-center rounded-xl transition-transform duration-300 group-hover:scale-110 ${bg} ${color}`}>
          {icon}
        </div>
        <div>
          <p className="text-2xl font-black text-primary tracking-tight leading-none mb-1">{count}</p>
