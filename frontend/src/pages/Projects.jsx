...
const calculateTimeProgress = (startStr, dueStr) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-primary flex items-center gap-2">
            <FolderKanban size={22} className="text-[var(--accent)]" />
            Workspaces
          </h1>
      ...
      {projects.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {projects.map((project) => {
            ...
            return (
              <div key={project.id} className="rounded-2xl card p-5 flex flex-col justify-between hover:-translate-y-1 hover:shadow-xl hover:border-[var(--accent-border)] transition-all duration-300 group">
                {/* Top section */}
                <div>
                  ...
                  <h2 className="text-lg font-bold text-primary group-hover:text-[var(--accent)] transition-colors duration-200">{project.name}</h2>
                ...
                <div className="mt-5 space-y-2">
                  ...
                  {(() => {
                    const progress = calculateTimeProgress(project.startDate, project.dueDate);
                    return (
                ...
                {/* Members + Action */}
                <div className="flex items-center justify-between mt-5 pt-4 border-t border-theme">
                  <div className="flex -space-x-2">
                    {members.slice(0, 4).map((m) => (
                      <div key={m.id} className="w-7 h-7 rounded-full bg-gradient-to-br from-violet-500 to-amber-500 flex items-center justify-center text-[10px] font-bold text-white ring-2 ring-[var(--bg)]" title={m.user.name}>
              ...
              <div className="space-y-2">
                {(selectedProject.members || []).map((m) => (
                  <div key={m.id} className="flex items-center gap-3 rounded-xl card px-4 py-3">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-amber-500 flex items-center justify-center text-xs font-bold text-white">
                      {m.user.name[0].toUpperCase()}
                    </div>
